...
export class BasketService {
  ...
  addItemToBasket(item: IProduct, quantity = 1) {
    ...
    const basket = this.getCurrentBasketValue() ?? this.createBusket();
  ...
  decrementItemQuantity(item: IBasketItem){
    const basket = this.getCurrentBasketValue();
    const foundItemIndex = basket.basketItems.findIndex(x => x.id === item.id);
    basket.basketItems[foundItemIndex].quantity > 1 ? basket.basketItems[foundItemIndex].quantity--
